...
def apply_from_job(job: dict) -> dict:
    ...
    source = str(job.get("cf_source") or "global").strip().lower()
    if source == "own":
        ...
        # "global" (or unset) — leave the env as-is; keys_sync already
        # populated it from /keys at worker boot.
        if os.environ.get("CLOUDFLARE_ACCOUNT_ID"):
            log.info(
                f"channel_cf: using GLOBAL Cloudflare creds "
                f"(source={source or 'unset'})"
            )
